...
namespace CSparse.Complex
{
    ...
    public class SparseMatrix : Matrix
    {
        ...
        public bool Axpy(Complex[] xx, Complex[] yy)
        {
        ...
        public override Matrix Transpose(bool values)
        {
            int pp, q, j;
            ...
            for (j = 0; j < n; j++)
            {
                for (pp = p[j]; pp < p[j + 1]; pp++)
                {
                    ...
                    if (Cx != null)
                    {
                        Cx[q] = values ? Complex.Conjugate(x[pp]) : x[pp];
                    }
                }
            }
        ...
        {
            Func<int, int, Complex, object, bool> cs_nonzero = (int i, int j, Complex aij, object other) =>
            {
                return (aij != 0);
            };
        ...
        {
            Func<int, int, Complex, object, bool> cs_tol = (int i, int j, Complex aij, object ftol) =>
            {
                return (Complex.Abs(aij) > (double)ftol);
            };
        ...
        public int Scatter(int j, Complex beta, int[] w, Complex[] xx, int mark, SparseMatrix C, int nzz)
        {
            int ii, pp;
            int[] Ci;
        ...
        public double Norm()
        {
            ...
            for (j = 0; j < n; j++)
            {
                for (s = 0, pp = p[j]; pp < p[j + 1]; pp++) s += Complex.Abs(x[pp]);
        ...
        public override Matrix Permute(int[] pinv, int[] q, bool values)
        {
            int t, j, k, nz = 0, m, n;
            int[] Ap, Ai, Cp, Ci;
            Complex[] Cx, Ax;
        ...
        /// <returns>nz, new number of entries in A, -1 on error</returns>
        public int Keep(Func<int, int, Complex, object, bool> fkeep, object other)
        {
            int j, pp, nzz = 0;
        ...
        public static SparseMatrix Add(SparseMatrix A, SparseMatrix B, Complex alpha, Complex beta)
        {
